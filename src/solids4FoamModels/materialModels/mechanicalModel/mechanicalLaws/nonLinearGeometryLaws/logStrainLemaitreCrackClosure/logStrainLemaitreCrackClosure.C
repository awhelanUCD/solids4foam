...
#include "logStrainLemaitreCrackClosure.H"
...
namespace Foam
{
    defineTypeNameAndDebug(logStrainLemaitreCrackClosure, 0);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, logStrainLemaitreCrackClosure, nonLinGeomMechLaw
    );

// * * * * * * * * * * * * * * Static Members  * * * * * * * * * * * * * * * //

    // Tolerance for Newton loop
    scalar logStrainLemaitreCrackClosure::LoopTol_ = 1e-4;

    // Maximum number of iterations for Newton loop
    label logStrainLemaitreCrackClosure::MaxNewtonIter_ = 200;

    // finiteDiff is the delta for finite difference differentiation
    scalar logStrainLemaitreCrackClosure::finiteDiff_ = 0.25e-6;

    // Store sqrt(2/3) as we use it often
    scalar logStrainLemaitreCrackClosure::sqrtTwoOverThree_ = ::sqrt(2.0/3.0);
...
Foam::symmTensor Foam::logStrainLemaitreCrackClosure::logm(const symmTensor& T)
{
...
Foam::symmTensor Foam::logStrainLemaitreCrackClosure::expm(const symmTensor& T)
{
...
void Foam::logStrainLemaitreCrackClosure::decompose
(
...
Foam::scalar Foam::logStrainLemaitreCrackClosure::macauley
(
  scalar x
)
{
...
void Foam::logStrainLemaitreCrackClosure::makeRelF()
{
    if (relFPtr_)
    {
        FatalErrorIn("void Foam::logStrainLemaitreCrackClosure::makeRelF()")
...
Foam::volTensorField& Foam::logStrainLemaitreCrackClosure::relF()
{
    if (!relFPtr_)
    {
        makeRelF();
...
void Foam::logStrainLemaitreCrackClosure::makeJ()
{
    if (JPtr_)
    {
        FatalErrorIn("void Foam::logStrainLemaitreCrackClosure::makeJ()")
            << "pointer already set" << abort(FatalError);
    }
...
Foam::volScalarField& Foam::logStrainLemaitreCrackClosure::J()
{
    if (!JPtr_)
    {
        makeJ();
...
void Foam::logStrainLemaitreCrackClosure::makeF()
{
    if (FPtr_)
    {
        FatalErrorIn("void Foam::logStrainLemaitreCrackClosure::makeF()")
            << "pointer already set" << abort(FatalError);
    }
...
Foam::volTensorField& Foam::logStrainLemaitreCrackClosure::F()
{
    if (!FPtr_)
    {
        makeF();
...
Foam::scalar Foam::logStrainLemaitreCrackClosure::calcZheta
(
...
void Foam::logStrainLemaitreCrackClosure::damage
(
...
) 
{
    ...
    //initialise positive and negative stress tensors
    symmTensor tauPositive, tauNegative;

    //trace of the stress tensor
    scalar traceTau = tr(tau);
    ...
    if ( triaxiality > (-1.0/3.0) && epsilonPEq > epsilonD_.value())
    {
        decompose
         ...
         scalar denomPositive = 1/(2*E_.value()*(pow(1-damageNonLocal,2.0)));

         //denominator associated with the negative stress tensor
         scalar denomNegative = h_/(2*E_.value()*(pow(1-h_*damageNonLocal,2.0)));

         YPos = denomPositive*((1.0 + nu_.value())*
               (tauPositive&&tauPositive)-nu_.value()*pow(macauley(traceTau), 2.0));
         YNeg = denomNegative*(
                                     (1.0 + nu_.value())*
                                     (tauNegative&&tauNegative)-nu_.value()*pow(macauley(-traceTau), 2.0)
                                     );
         ...
         else if (lemaitreType_ == "Malcher")
         {
             ...
             sDenom = s33_.value()
                     /(3.0*mag(triaxiality) + (s33_.value()/s00_.value())*(1-pow(zheta,2.0)));
         }
...
void Foam::logStrainLemaitreCrackClosure::smallStrainReturnMap
(
...
) const
{
	...
	if (fTrial > 0.0) 
	{
        ...
        do
        {
    	    ...
    	    fTrial = qTrial - 3*mu_.value()*DLambda/(1-damageNonLocal)
...
Foam::tmp<Foam::volScalarField> Foam::logStrainLemaitreCrackClosure::Ibar
(
...
Foam::tmp<Foam::surfaceScalarField> Foam::logStrainLemaitreCrackClosure::Ibar
(
...
Foam::logStrainLemaitreCrackClosure::logStrainLemaitreCrackClosure
(
    const word& name,
    const fvMesh& mesh,
...
:
    ...
    h_(dict.lookupOrDefault<scalar>("h", 0.2)),
    ...
    )
{
    ...
    else
    {
        FatalErrorIn
        (
            "logStrainLemaitreCrackClosure::logStrainLemaitreCrackClosure::()"
...
Foam::tmp<Foam::volScalarField> Foam::logStrainLemaitreCrackClosure::rho() const
{
...
Foam::logStrainLemaitreCrackClosure::impK() const
{
...
void Foam::logStrainLemaitreCrackClosure::correct(volSymmTensorField& sigma)
{
    ...
    symmTensorField& sigmaPositiveI =sigmaPositive_.internalField();
    symmTensorField& sigmaNegativeI =sigmaNegative_.internalField();
    ...
    forAll(relFI, cellI)
    {
       ...
       DEpsilonPI[cellI] = sqrt(3.0/2.0)*DLambdaI[cellI]/(1-damageNonLocalI[cellI])*(plasticN);
       epsilonElI[cellI] = Ee-DEpsilonPI[cellI];
       tauI[cellI] = (1-damageNonLocalI[cellI])
    ...
    forAll(F().boundaryField(), patchI)
    {
        ...
        symmTensorField& sigmaPositiveP = sigmaPositive_.boundaryField()[patchI];
        symmTensorField& sigmaNegativeP = sigmaNegative_.boundaryField()[patchI];
        ...
        forAll(FP, faceI)
        {
            ...
            tauP[faceI] = (1-damageNonLocalP[faceI])
                          *(2*mu_.value()*dev(epsilonElP[faceI]) + K_.value()*tr(epsilonElP[faceI])*I);
            ...
            {
                scalar sigmaEq = sqrt(3.0/2.0)*mag(dev(tauP[faceI]))/(1-damageNonLocalP[faceI]);
                scalar triaxiality = (1.0/3.0)*(tr(tauP[faceI])/(1-damageNonLocalP[faceI]))/sigmaEq;
...
void Foam::logStrainLemaitreCrackClosure::correct(surfaceSymmTensorField& sigma)
{
...
Foam::scalar Foam::logStrainLemaitreCrackClosure::residual()
{
...
void Foam::logStrainLemaitreCrackClosure::updateTotalFields()
{
...
Foam::scalar Foam::logStrainLemaitreCrackClosure::newDeltaT()
{
