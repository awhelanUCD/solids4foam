...
#include "logStrainLemaitreDamage.H"
...
namespace Foam
{
    defineTypeNameAndDebug(logStrainLemaitreDamage, 0);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, logStrainLemaitreDamage, nonLinGeomMechLaw
    );

// * * * * * * * * * * * * * * Static Members  * * * * * * * * * * * * * * * //

    // Tolerance for Newton loop
    scalar logStrainLemaitreDamage::LoopTol_ = 1e-4;

    // Maximum number of iterations for Newton loop
    label logStrainLemaitreDamage::MaxNewtonIter_ = 200;

    // finiteDiff is the delta for finite difference differentiation
    scalar logStrainLemaitreDamage::finiteDiff_ = 0.25e-6;

    // Store sqrt(2/3) as we use it often
    scalar logStrainLemaitreDamage::sqrtTwoOverThree_ = ::sqrt(2.0/3.0);
...
Foam::symmTensor Foam::logStrainLemaitreDamage::logm(const symmTensor& T)
{
...
Foam::symmTensor Foam::logStrainLemaitreDamage::expm(const symmTensor& T)
{
...
void Foam::logStrainLemaitreDamage::decompose
(
...
Foam::scalar Foam::logStrainLemaitreDamage::macauley
(
  scalar x
)
{
...
void Foam::logStrainLemaitreDamage::makeRelF()
{
    if (relFPtr_)
    {
        FatalErrorIn("void Foam::logStrainLemaitreDamage::makeRelF()")
...
Foam::volTensorField& Foam::logStrainLemaitreDamage::relF()
{
    if (!relFPtr_)
    {
        makeRelF();
...
void Foam::logStrainLemaitreDamage::makeJ()
{
    if (JPtr_)
    {
        FatalErrorIn("void Foam::logStrainLemaitreDamage::makeJ()")
            << "pointer already set" << abort(FatalError);
    }
...
Foam::volScalarField& Foam::logStrainLemaitreDamage::J()
{
    if (!JPtr_)
    {
        makeJ();
...
void Foam::logStrainLemaitreDamage::makeF()
{
    if (FPtr_)
    {
        FatalErrorIn("void Foam::logStrainLemaitreDamage::makeF()")
            << "pointer already set" << abort(FatalError);
    }
...
Foam::volTensorField& Foam::logStrainLemaitreDamage::F()
{
    if (!FPtr_)
    {
        makeF();
...
Foam::scalar Foam::logStrainLemaitreDamage::calcZheta
(
...
void Foam::logStrainLemaitreDamage::damage
(
...
) 
{
    ...
    if ( triaxiality> (-1.0/3.0) && epsilonPEq > epsilonD_.value())
    {
         ...
         else if (lemaitreType_ == "Malcher")
         {
             ...
             sDenom = s33_.value()/(3.0*mag(triaxiality) + (s33_.value()/s00_.value())*(1-pow(zheta,2.0)));
         }
...
void Foam::logStrainLemaitreDamage::smallStrainReturnMap
(
...
Foam::tmp<Foam::volScalarField> Foam::logStrainLemaitreDamage::Ibar
(
...
Foam::tmp<Foam::surfaceScalarField> Foam::logStrainLemaitreDamage::Ibar
(
...
Foam::logStrainLemaitreDamage::logStrainLemaitreDamage
(
    const word& name,
    const fvMesh& mesh,
    ...
    )
{
    ...
    else
    {
        FatalErrorIn
        (
            "logStrainLemaitreDamage::logStrainLemaitreDamage::()"
...
Foam::tmp<Foam::volScalarField> Foam::logStrainLemaitreDamage::rho() const
{
...
Foam::logStrainLemaitreDamage::impK() const
{
...
void Foam::logStrainLemaitreDamage::correct(volSymmTensorField& sigma)
{
...
void Foam::logStrainLemaitreDamage::correct(surfaceSymmTensorField& sigma)
{
...
Foam::scalar Foam::logStrainLemaitreDamage::residual()
{
...
void Foam::logStrainLemaitreDamage::updateTotalFields()
{
...
Foam::scalar Foam::logStrainLemaitreDamage::newDeltaT()
{
