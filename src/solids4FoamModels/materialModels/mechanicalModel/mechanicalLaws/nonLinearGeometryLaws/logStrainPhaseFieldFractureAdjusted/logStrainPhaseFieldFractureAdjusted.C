...
#include "logStrainPhaseFieldFractureAdjusted.H"
...
namespace Foam
{
    defineTypeNameAndDebug(logStrainPhaseFieldFractureAdjusted, 0);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, logStrainPhaseFieldFractureAdjusted, nonLinGeomMechLaw
    );

// * * * * * * * * * * * * * * Static Members  * * * * * * * * * * * * * * * //

    // Tolerance for Newton loop
    scalar logStrainPhaseFieldFractureAdjusted::LoopTol_ = 1e-4;

    // Maximum number of iterations for Newton loop
    label logStrainPhaseFieldFractureAdjusted::MaxNewtonIter_ = 200;

    // finiteDiff is the delta for finite difference differentiation
    scalar logStrainPhaseFieldFractureAdjusted::finiteDiff_ = 0.25e-6;

    // Store sqrt(2/3) as we use it often
    scalar logStrainPhaseFieldFractureAdjusted::sqrtTwoOverThree_ = ::sqrt(2.0/3.0);
...
Foam::symmTensor Foam::logStrainPhaseFieldFractureAdjusted::logm(const symmTensor& T)
{
...
Foam::symmTensor Foam::logStrainPhaseFieldFractureAdjusted::expm(const symmTensor& T)
{
...
void Foam::logStrainPhaseFieldFractureAdjusted::makeRelF()
{
    if (relFPtr_)
    {
        FatalErrorIn("void Foam::logStrainPhaseFieldFractureAdjusted::makeRelF()")
...
Foam::volTensorField& Foam::logStrainPhaseFieldFractureAdjusted::relF()
{
    if (!relFPtr_)
    {
        makeRelF();
...
void Foam::logStrainPhaseFieldFractureAdjusted::makeJ()
{
    if (JPtr_)
    {
        FatalErrorIn("void Foam::logStrainPhaseFieldFractureAdjusted::makeJ()")
            << "pointer already set" << abort(FatalError);
    }
...
Foam::volScalarField& Foam::logStrainPhaseFieldFractureAdjusted::J()
{
    if (!JPtr_)
    {
        makeJ();
...
void Foam::logStrainPhaseFieldFractureAdjusted::makeF()
{
    if (FPtr_)
    {
        FatalErrorIn("void Foam::logStrainPhaseFieldFractureAdjusted::makeF()")
            << "pointer already set" << abort(FatalError);
    }
...
Foam::volTensorField& Foam::logStrainPhaseFieldFractureAdjusted::F()
{
    if (!FPtr_)
    {
        makeF();
...
void Foam::logStrainPhaseFieldFractureAdjusted::smallStrainReturnMap
(
...
) const
{
    ...
    while ((mag(fTrial) > 1e-4) && ++i < MaxNewtonIter_);
...
Foam::tmp<Foam::volScalarField> Foam::logStrainPhaseFieldFractureAdjusted::Ibar
(
...
Foam::tmp<Foam::surfaceScalarField> Foam::logStrainPhaseFieldFractureAdjusted::Ibar
(
...
Foam::logStrainPhaseFieldFractureAdjusted::logStrainPhaseFieldFractureAdjusted
(
    const word& name,
    const fvMesh& mesh,
...
:
    ...
    sigmaY0_("zero", dimPressure, 0.0),
    H_("zero", dimPressure, 0.0),
    ...
    )
{
    ...
    sigmaY0_ = dimensionedScalar(dict.lookup("sigmaY0"));
    ...
    else
    {
        FatalErrorIn
        (
            "logStrainPhaseFieldFractureAdjusted::logStrainPhaseFieldFractureAdjusted::()"
...
Foam::tmp<Foam::volScalarField> Foam::logStrainPhaseFieldFractureAdjusted::rho() const
{
...
Foam::logStrainPhaseFieldFractureAdjusted::impK() const
{
...
void Foam::logStrainPhaseFieldFractureAdjusted::calcPhase()
{
    ...
    volScalarField coeff = HStrainEnergy_ + effPlasticStrainEnergy;
    ...
    (
      ...
      ==   
       (4.0*charLength_*coeff)/(Gc_*diml)
       -fvm::Sp((4.0*charLength_*coeff)/(Gc_*diml), Dpf_)
    );
...
void Foam::logStrainPhaseFieldFractureAdjusted::correct(volSymmTensorField& sigma)
{
    ...
    volScalarField gc_ = pow(1.0 - Dpf_, 2.0);//set crack degredation function
...
void Foam::logStrainPhaseFieldFractureAdjusted::correct(surfaceSymmTensorField& sigma)
{
...
Foam::scalar Foam::logStrainPhaseFieldFractureAdjusted::residual()
{
...
void Foam::logStrainPhaseFieldFractureAdjusted::updateTotalFields()
{
...
Foam::scalar Foam::logStrainPhaseFieldFractureAdjusted::newDeltaT()
{
